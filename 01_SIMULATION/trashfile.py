...
class Last_dummy(Thread):
    ...
    def run(self):
        ...
        load_profile_list = [load_profile_we1, load_profile_we2, load_profile_we3, load_profile_we4, load_profile_we5,
                             load_profile_we6, load_profile_we7, load_profile_we8, load_profile_we9, load_profile_we10,
                             load_profile_we11, load_profile_we12, load_profile_we13, load_profile_we14,
                             load_profile_we15, load_profile_we16, load_profile_we17, load_profile_we18,
                             load_profile_we19, load_profile_we20, load_profile_we21, load_profile_we22,
                             load_profile_we23, load_profile_we24]
...
class Added_segment(Thread):
    ...
    def strategy_segment(self):
        ...
        P_grid_real = sum(WE.P_Netz_v for WE in
                          Wohneinheiten)  # P_grid_real - Tatsächlicher Netzbezug (abzüglich PV-Überschussscheibe)
...
class MomentanwertDB(Thread):
    ...
    def Momentanwertdatenbank(self):
        ...
        value_list = [
            (
            Timestamp, 'Geraetewerte', round(BSS.E_bat_device, 2), BSS.SoC, BSS.P_BSS_device, P_load_sum, P_Wallbox_sum,
            P_pv_sum, round(P_grid_real, 2)),
            (Timestamp, 'Rechenwerte', round(E_bat_sum, 2), BSS.SoC, P_BSS_sum, P_load_sum, P_Wallbox_sum, P_pv_sum,
             round(P_grid_sum, 2)),
            (Timestamp, WE1.Wohneinheit, round(WE1.E_bat_v, 2), WE1.SoC_v, WE1.P_bat_v, WE1.P_load_v, WE1.P_Wallbox,
             WE1.P_pv_usage, round(WE1.P_Netz_v, 2)),
            (Timestamp, WE2.Wohneinheit, round(WE2.E_bat_v, 2), WE2.SoC_v, WE2.P_bat_v, WE2.P_load_v, WE2.P_Wallbox,
             WE2.P_pv_usage, round(WE2.P_Netz_v, 2)),
            (Timestamp, WE3.Wohneinheit, round(WE3.E_bat_v, 2), WE3.SoC_v, WE3.P_bat_v, WE3.P_load_v, WE3.P_Wallbox,
             WE3.P_pv_usage, round(WE3.P_Netz_v, 2)),
            (Timestamp, WE4.Wohneinheit, round(WE4.E_bat_v, 2), WE4.SoC_v, WE4.P_bat_v, WE4.P_load_v, WE4.P_Wallbox,
             WE4.P_pv_usage, round(WE4.P_Netz_v, 2)),
            (Timestamp, WE5.Wohneinheit, round(WE5.E_bat_v, 2), WE5.SoC_v, WE5.P_bat_v, WE5.P_load_v, WE5.P_Wallbox,
             WE5.P_pv_usage, round(WE5.P_Netz_v, 2)),
            (Timestamp, WE6.Wohneinheit, round(WE6.E_bat_v, 2), WE6.SoC_v, WE6.P_bat_v, WE6.P_load_v, WE6.P_Wallbox,
             WE6.P_pv_usage, round(WE6.P_Netz_v, 2)),
            (Timestamp, WE7.Wohneinheit, round(WE7.E_bat_v, 2), WE7.SoC_v, WE7.P_bat_v, WE7.P_load_v, WE7.P_Wallbox,
             WE7.P_pv_usage, round(WE7.P_Netz_v, 2)),
            (Timestamp, WE8.Wohneinheit, round(WE8.E_bat_v, 2), WE8.SoC_v, WE8.P_bat_v, WE8.P_load_v, WE8.P_Wallbox,
             WE8.P_pv_usage, round(WE8.P_Netz_v, 2)),
            (Timestamp, WE9.Wohneinheit, round(WE9.E_bat_v, 2), WE9.SoC_v, WE9.P_bat_v, WE9.P_load_v, WE9.P_Wallbox,
             WE9.P_pv_usage, round(WE9.P_Netz_v, 2)),
            (Timestamp, WE10.Wohneinheit, round(WE10.E_bat_v, 2), WE10.SoC_v, WE10.P_bat_v, WE10.P_load_v,
             WE10.P_Wallbox, WE10.P_pv_usage, round(WE10.P_Netz_v, 2)),
            (Timestamp, WE11.Wohneinheit, round(WE11.E_bat_v, 2), WE11.SoC_v, WE11.P_bat_v, WE11.P_load_v,
             WE11.P_Wallbox, WE11.P_pv_usage, round(WE11.P_Netz_v, 2)),
            (Timestamp, WE12.Wohneinheit, round(WE12.E_bat_v, 2), WE12.SoC_v, WE12.P_bat_v, WE12.P_load_v,
             WE12.P_Wallbox, WE12.P_pv_usage, round(WE12.P_Netz_v, 2)),
            (Timestamp, WE13.Wohneinheit, round(WE13.E_bat_v, 2), WE13.SoC_v, WE13.P_bat_v, WE13.P_load_v,
             WE13.P_Wallbox, WE13.P_pv_usage, round(WE13.P_Netz_v, 2)),
            (Timestamp, WE14.Wohneinheit, round(WE14.E_bat_v, 2), WE14.SoC_v, WE14.P_bat_v, WE14.P_load_v,
             WE14.P_Wallbox, WE14.P_pv_usage, round(WE14.P_Netz_v, 2)),
            (Timestamp, WE15.Wohneinheit, round(WE15.E_bat_v, 2), WE15.SoC_v, WE15.P_bat_v, WE15.P_load_v,
             WE15.P_Wallbox, WE15.P_pv_usage, round(WE15.P_Netz_v, 2)),
            (Timestamp, WE16.Wohneinheit, round(WE16.E_bat_v, 2), WE16.SoC_v, WE16.P_bat_v, WE16.P_load_v,
             WE16.P_Wallbox, WE16.P_pv_usage, round(WE16.P_Netz_v, 2)),
            (Timestamp, WE17.Wohneinheit, round(WE17.E_bat_v, 2), WE17.SoC_v, WE17.P_bat_v, WE17.P_load_v,
             WE17.P_Wallbox, WE17.P_pv_usage, round(WE17.P_Netz_v, 2)),
            (Timestamp, WE18.Wohneinheit, round(WE18.E_bat_v, 2), WE18.SoC_v, WE18.P_bat_v, WE18.P_load_v,
             WE18.P_Wallbox, WE18.P_pv_usage, round(WE18.P_Netz_v, 2)),
            (Timestamp, WE19.Wohneinheit, round(WE19.E_bat_v, 2), WE19.SoC_v, WE19.P_bat_v, WE19.P_load_v,
             WE19.P_Wallbox, WE19.P_pv_usage, round(WE19.P_Netz_v, 2)),
            (Timestamp, WE20.Wohneinheit, round(WE20.E_bat_v, 2), WE20.SoC_v, WE20.P_bat_v, WE20.P_load_v,
             WE20.P_Wallbox, WE20.P_pv_usage, round(WE20.P_Netz_v, 2)),
            (Timestamp, WE21.Wohneinheit, round(WE21.E_bat_v, 2), WE21.SoC_v, WE21.P_bat_v, WE21.P_load_v,
             WE21.P_Wallbox, WE21.P_pv_usage, round(WE21.P_Netz_v, 2)),
            (Timestamp, WE22.Wohneinheit, round(WE22.E_bat_v, 2), WE22.SoC_v, WE22.P_bat_v, WE22.P_load_v,
             WE22.P_Wallbox, WE22.P_pv_usage, round(WE22.P_Netz_v, 2)),
            (Timestamp, WE23.Wohneinheit, round(WE23.E_bat_v, 2), WE23.SoC_v, WE23.P_bat_v, WE23.P_load_v,
             WE23.P_Wallbox, WE23.P_pv_usage, round(WE23.P_Netz_v, 2)),
            (Timestamp, WE24.Wohneinheit, round(WE24.E_bat_v, 2), WE24.SoC_v, WE24.P_bat_v, WE24.P_load_v,
             WE24.P_Wallbox, WE24.P_pv_usage, round(WE24.P_Netz_v, 2)),
            (Timestamp, 'Zusatzsegment', round(Provider_segment.E_bat_segment, 2),
             round(Provider_segment.SoC_Segment, 2), round(Provider_segment.P_bat_segment, 2),
        ...
        curSQ.executemany("INSERT OR REPLACE INTO Tabelle1 "
                          "(Timestamp, Name, E_BSS, SoC,P_BSS, P_Last, P_Wallbox,P_PV_Nutz, P_Netz) VALUES (?,?,?,?,?,?,?,?,?)",
                          (value_list))
        conSQ.commit()
...
class ZeitreihenDB(Thread):
    ...
    def CSV_Daten(self):
        ...
        Device_list = [Timestamp, round(BSS.E_bat_device, 2), BSS.SoC, BSS.P_BSS_device, P_load_sum, P_Wallbox_sum,
                       P_pv_sum, P_grid_real]
        Calc_sum_list = [Timestamp, E_bat_sum, BSS.SoC, P_BSS_sum, P_load_sum, P_Wallbox_sum, P_pv_sum, P_grid_sum]

        csv_list_we1 = [Timestamp, round(WE1.E_bat_v, 2), WE1.SoC_v, WE1.P_bat_v, WE1.P_load_v, WE1.P_Wallbox,
                        WE1.P_pv_v, round(WE1.P_Netz_v, 2)]
        csv_list_we2 = [Timestamp, round(WE2.E_bat_v, 2), WE2.SoC_v, WE2.P_bat_v, WE2.P_load_v, WE2.P_Wallbox,
                        WE2.P_pv_v, round(WE2.P_Netz_v, 2)]
        csv_list_we3 = [Timestamp, round(WE3.E_bat_v, 2), WE3.SoC_v, WE3.P_bat_v, WE3.P_load_v, WE3.P_Wallbox,
                        WE3.P_pv_v, round(WE3.P_Netz_v, 2)]
        csv_list_we4 = [Timestamp, round(WE4.E_bat_v, 2), WE4.SoC_v, WE4.P_bat_v, WE4.P_load_v, WE4.P_Wallbox,
                        WE4.P_pv_v, round(WE4.P_Netz_v, 2)]
        csv_list_we5 = [Timestamp, round(WE5.E_bat_v, 2), WE5.SoC_v, WE5.P_bat_v, WE5.P_load_v, WE5.P_Wallbox,
                        WE5.P_pv_v, round(WE5.P_Netz_v, 2)]
        csv_list_we6 = [Timestamp, round(WE6.E_bat_v, 2), WE6.SoC_v, WE6.P_bat_v, WE6.P_load_v, WE6.P_Wallbox,
                        WE6.P_pv_v, round(WE6.P_Netz_v, 2)]
        csv_list_we7 = [Timestamp, round(WE7.E_bat_v, 2), WE7.SoC_v, WE7.P_bat_v, WE7.P_load_v, WE7.P_Wallbox,
                        WE7.P_pv_v, round(WE7.P_Netz_v, 2)]
        csv_list_we8 = [Timestamp, round(WE8.E_bat_v, 2), WE8.SoC_v, WE8.P_bat_v, WE8.P_load_v, WE8.P_Wallbox,
                        WE8.P_pv_v, round(WE8.P_Netz_v, 2)]
        csv_list_we9 = [Timestamp, round(WE9.E_bat_v, 2), WE9.SoC_v, WE9.P_bat_v, WE9.P_load_v, WE9.P_Wallbox,
                        WE9.P_pv_v, round(WE9.P_Netz_v, 2)]
        csv_list_we10 = [Timestamp, round(WE10.E_bat_v, 2), WE10.SoC_v, WE10.P_bat_v, WE10.P_load_v, WE10.P_Wallbox,
                         WE10.P_pv_v, round(WE10.P_Netz_v, 2)]
        csv_list_we11 = [Timestamp, round(WE11.E_bat_v, 2), WE11.SoC_v, WE11.P_bat_v, WE11.P_load_v, WE11.P_Wallbox,
                         WE11.P_pv_v, round(WE11.P_Netz_v, 2)]
        csv_list_we12 = [Timestamp, round(WE12.E_bat_v, 2), WE12.SoC_v, WE12.P_bat_v, WE12.P_load_v, WE12.P_Wallbox,
                         WE12.P_pv_v, round(WE12.P_Netz_v, 2)]
        csv_list_we13 = [Timestamp, round(WE13.E_bat_v, 2), WE13.SoC_v, WE13.P_bat_v, WE13.P_load_v, WE13.P_Wallbox,
                         WE13.P_pv_v, round(WE13.P_Netz_v, 2)]
        csv_list_we14 = [Timestamp, round(WE14.E_bat_v, 2), WE14.SoC_v, WE14.P_bat_v, WE14.P_load_v, WE14.P_Wallbox,
                         WE14.P_pv_v, round(WE14.P_Netz_v, 2)]
        csv_list_we15 = [Timestamp, round(WE15.E_bat_v, 2), WE15.SoC_v, WE15.P_bat_v, WE15.P_load_v, WE15.P_Wallbox,
                         WE15.P_pv_v, round(WE15.P_Netz_v, 2)]
        csv_list_we16 = [Timestamp, round(WE16.E_bat_v, 2), WE16.SoC_v, WE16.P_bat_v, WE16.P_load_v, WE16.P_Wallbox,
                         WE16.P_pv_v, round(WE16.P_Netz_v, 2)]
        csv_list_we17 = [Timestamp, round(WE17.E_bat_v, 2), WE17.SoC_v, WE17.P_bat_v, WE17.P_load_v, WE17.P_Wallbox,
                         WE17.P_pv_v, round(WE17.P_Netz_v, 2)]
        csv_list_we18 = [Timestamp, round(WE18.E_bat_v, 2), WE18.SoC_v, WE18.P_bat_v, WE18.P_load_v, WE18.P_Wallbox,
                         WE18.P_pv_v, round(WE18.P_Netz_v, 2)]
        csv_list_we19 = [Timestamp, round(WE19.E_bat_v, 2), WE19.SoC_v, WE19.P_bat_v, WE19.P_load_v, WE19.P_Wallbox,
                         WE19.P_pv_v, round(WE19.P_Netz_v, 2)]
        csv_list_we20 = [Timestamp, round(WE20.E_bat_v, 2), WE20.SoC_v, WE20.P_bat_v, WE20.P_load_v, WE20.P_Wallbox,
                         WE20.P_pv_v, round(WE20.P_Netz_v, 2)]
        csv_list_we21 = [Timestamp, round(WE21.E_bat_v, 2), WE21.SoC_v, WE21.P_bat_v, WE21.P_load_v, WE21.P_Wallbox,
                         WE21.P_pv_v, round(WE21.P_Netz_v, 2)]
        csv_list_we22 = [Timestamp, round(WE22.E_bat_v, 2), WE22.SoC_v, WE22.P_bat_v, WE22.P_load_v, WE22.P_Wallbox,
                         WE22.P_pv_v, round(WE22.P_Netz_v, 2)]
        csv_list_we23 = [Timestamp, round(WE23.E_bat_v, 2), WE23.SoC_v, WE23.P_bat_v, WE23.P_load_v, WE23.P_Wallbox,
                         WE23.P_pv_v, round(WE23.P_Netz_v, 2)]
        csv_list_we24 = [Timestamp, round(WE24.E_bat_v, 2), WE24.SoC_v, WE24.P_bat_v, WE24.P_load_v, WE24.P_Wallbox,
                         WE24.P_pv_v, round(WE24.P_Netz_v, 2)]

        # SQlite-Datenbank wird erstellt
        connectSQ = sqlite3.connect(path2)
        cursorSQ = connectSQ.cursor()

        WE_list = [csv_list_we1, csv_list_we2, csv_list_we3, csv_list_we4, csv_list_we5, csv_list_we6, csv_list_we7,
                   csv_list_we8, csv_list_we9, csv_list_we10,
                   csv_list_we11, csv_list_we12, csv_list_we13, csv_list_we14, csv_list_we15, csv_list_we16,
                   csv_list_we17, csv_list_we18, csv_list_we19, csv_list_we20, csv_list_we21, csv_list_we22,
                   csv_list_we23, csv_list_we24]
...
class BSS_virtuell(Thread):
    ...
    def set_SOP(self):
        ...
        self.P_pv_v = round(self.P_pv_v,
                            2)  # Macht nichts außer Wert zu runden -> Achtung, später nur für Anzeige in DB runden, Rechnungswerte werden verfälscht!
    ...
    def strategy_ueberschussladen(self):
        if self.E_bat_v <= self.E_bat_v_min:
            ...
            self.P_bat_v = round(self.P_load_v + self.P_Wallbox - self.P_pv_v,
                                 2)  # bei PV-Überschuss negativer Wert -> Speicher laden!
    ...
    def strategy_P_BSS_discharge_limit(self):
        if self.E_bat_v <= self.E_bat_v_min:
            ...
            if (self.P_load_v + self.P_Wallbox - self.P_pv_v) < self.P_bat_v_max:
                ...
                P_BSS_Mehrbedarf = (self.P_Residual_phy) - (
                    sum(P_res_v_emptyWE_sum)) - BSS.P_BSS_discharge_max  # Ermittelt nötigen Netzbezug,  + sum(P_EV_v_emptyWE_sum)
                self.countWE = 1
                ...
                if ((
                            self.P_load_v + self.P_Wallbox - self.P_pv_v) - P_AbzugsleistungProWE) <= self.P_bat_v_max:  # Soll nicht unter 2,7 kW Grenze gedrosselt werden
                ...
                for WE in Wohneinheiten:
                    if WE.Participation_status == 0:
                        ...
                        if BSS.E_bat_device <= BSS.E_bat_min:
                            ...
                            continue
                        elif (P_load_sum + P_Wallbox_sum - P_pv_sum) - (
                        sum(P_res_v_emptyWE_sum)) >= BSS.P_BSS_discharge_max:  # + sum(P_EV_v_emptyWE_sum)
